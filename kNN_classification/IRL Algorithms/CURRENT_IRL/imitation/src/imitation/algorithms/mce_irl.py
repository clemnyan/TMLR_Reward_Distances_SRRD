...
import torch as th
from seals_dir.src.seals import base_envs
from stable__baselines3.stable__baselines3.common import policies
...
class TabularPolicy(policies.BasePolicy):
    ...
    def predict(
        ...
        for obs, t in zip(observation, timesteps):
            p = self.observation_space.contains(obs)
            q = self.observation_space

            assert self.observation_space.contains(obs), "illegal state"
            dist = self.pi[t, obs, :]
            #print("obs = ", obs, "   t = ", t)
            if deterministic:
                print(int(dist.argmax))
                ...
                d1 = dist**5
                d1/= sum(d1)
                print("   d1 = ", d1, "    sum(d1) = ", sum(d1))
                #m = [self.rng.choice(len(d1), p=d1) for i in range(10)]
                #print("    m = ", m)
                #actions.append(self.rng.choice(len(dist), p=dist))
                actions.append(self.rng.choice(len(d1), p=d1))
...
class MCEIRL(base.DemonstrationAlgorithm[types.TransitionsMinimal]):
    ...
    def __init__(
        ...
        discount: float = 1.0,
        linf_eps: float = 0.2,   # high value but its ok!
    ...
    def _train_step(self, obs_mat: th.Tensor) -> Tuple[np.ndarray, np.ndarray]:
        ...
        #pr = transform_Rews(self.env.observation_matrix, predicted_r_np)
        #print("pr = ", pr)
        #sys.exit()
        # canonicalize the reward
        #predicted_r_np = canonicalize_V(pr, 0.7)
    ...
    def train(self, max_iter: int = 100) -> np.ndarray:
        """Runs MCE IRL.
        ...
        print("maxent_iter = ", max_iter)
        
        V_soft, Q_soft, pi = mce_partition_fh(self.env, reward=predicted_r_np, \
            discount=self.discount)
        ...
        return visitations, V_soft, Q_soft, iteration_count
