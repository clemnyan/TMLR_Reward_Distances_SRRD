...
import tqdm
from stable__baselines3.stable__baselines3.common import (
    base_class,
    distributions,
    on_policy_algorithm,
    policies,
...
from stable__baselines3.stable__baselines3.sac import policies as sac_policies
    ...
    def _make_disc_train_batches(
        ...
        *,
        ...
        for start in range(0, batch_size, self.demo_minibatch_size):
            ...
            e_batch, g_batch =  expert_batch["obs"], gen_batch["obs"]
            if e_batch.shape != g_batch.shape:
                new_e_batch = np.array([self.venv.observation_matrix[i] for i in e_batch])
                e_batch = new_e_batch.reshape(g_batch.shape)
                obs = np.concatenate([e_batch, g_batch])
            else:
                obs = np.concatenate([expert_batch["obs"], gen_batch["obs"]])

            acts = np.concatenate([expert_batch["acts"], gen_batch["acts"]])

            en_batch, gn_batch =  expert_batch["next_obs"], gen_batch["next_obs"]
            if en_batch.shape != gn_batch.shape:
                new_en_batch = np.array([self.venv.observation_matrix[i] for i in en_batch])
                en_batch = new_en_batch.reshape(gn_batch.shape)
                next_obs = np.concatenate([en_batch, gn_batch])
            else:
