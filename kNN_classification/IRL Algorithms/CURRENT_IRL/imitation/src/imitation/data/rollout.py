...
import numpy as np
from stable__baselines3.stable__baselines3.common.base_class import BaseAlgorithm
from stable__baselines3.stable__baselines3.common.policies import BasePolicy
from stable__baselines3.stable__baselines3.common.utils import check_for_correct_spaces
from stable__baselines3.stable__baselines3.common.vec_env import VecEnv

from gymnasium import spaces
import imitation.src.imitation.data.types as types
...
class TrajectoryAccumulator:
    ...
    def add_steps_and_auto_finish_1(
        self,
        acts: np.ndarray,
        obs: np.ndarray,
        rews: np.ndarray,
...
def policy_to_callable(
    ...
    if policy is None:
        ...
        def get_actions(
            ...
            observation_matrix: Optional[np.ndarray],
        ) -> Tuple[np.ndarray, Optional[Tuple[np.ndarray, ...]]]:
            ...
            if venv.num_envs == 1:
                if (states is not None) and (observations is not None):
                    if isinstance(states, tuple):
                        states = states[0]

                if policy.observation_space != venv.observation_space:
                    observations = np.array([observation_matrix[i] for i in observations])
                    #print("obvs = ", observations)

            #print("is_box = ", isinstance(venv.observation_space, spaces.Box))
            #print("obs space = ", venv.observation_space)
            #print("obs  = ", observations)
            #print("policy.obs = ", policy.observation_space)
            #print("policy = ", policy)
            #print("im here mofos")
...
def generate_trajectories(
    ...
    *,
    ...
    while np.any(active):
        try:
            #print("a")
            acts, state = get_actions(obs, state, dones, deterministic_policy, \
                observation_matrix)
        except:
            #print("bb2")
            #acts, _ = get_actions(obs, state, dones, deterministic_policy, \
            #    observation_matrix)

            acts, _ = get_actions(obs, state, [])
...
def generate_without_policy(
    traj_store: List[List[List[int]]],
) -> Sequence[types.TrajectoryWithRew]:
    ...
    for traj in traj_store:
        ...
        for s, a, sp in traj:
            ...
            obs.append(s)
            rews.append(0.0)
            infos.append({'old_state': s, 'new_state': sp, 'TimeLimit.truncated': False})

        k = infos[-1]
        k = {'old_state': k['old_state'], 'new_state': k['new_state'], \
            'TimeLimit.truncated': False, 'terminal_observation':k['new_state']}
        infos[-1] = k
        obs.append(sp)
        acts, obs, rews = np.array(acts), np.array(obs), np.array(rews)

        terminal = False
        new_trajs = types.TrajectoryWithRew(obs, acts, infos, terminal, rews)
