...
from stable__baselines3.stable__baselines3.common.vec_env import VecEnv, VecEnvWrapper
...
class BufferingWrapper(VecEnvWrapper):
    ...
    def reset(self, **kwargs):
        ...
        if self.venv.num_envs == 1:
            self._timesteps = np.zeros((1, ), dtype = int)
        else:
    ...
    def step_wait(self, actions):
        ...
        if self.venv.num_envs == 1:
            ...
            #print(self.venv.step(actions[0]))
            #sys.exit()
            try:
                obs, rews, terminated, truncated, infos = self.venv.step(actions[0])
            except:
                obs, rews, terminated, truncated, infos = self.venv.step(actions)

            dones = terminated or truncated
            #infos = {'TimeLimit.truncated': truncated}
            try:
                infos['old_state'] = self.venv.observation_matrix[np.array([infos['old_state']])]
                infos['new_state'] = self.venv.observation_matrix[np.array([infos['new_state']])]

                obs, rews, dones, infos = np.array([obs]), np.array([rews]), \
