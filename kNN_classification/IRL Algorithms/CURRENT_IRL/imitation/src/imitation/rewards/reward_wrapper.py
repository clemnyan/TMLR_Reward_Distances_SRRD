...
import numpy as np
from stable__baselines3.stable__baselines3.common import callbacks
from stable__baselines3.stable__baselines3.common import logger as sb_logger
from stable__baselines3.stable__baselines3.common import vec_env
...
class RewardVecEnvWrapper(vec_env.VecEnvWrapper):
    ...
    def step_wait(self, actions):
        obs, old_rews, dones, infos = self.venv.step_wait(actions)
        ...
        for single_obs, single_done, single_infos in zip(obs, dones, infos):
            if single_done:
                ...
                k_terminal = "terminal_observation"

                if k_terminal in single_infos:
                    single_obs = single_infos[k_terminal]
                else:
                    single_obs = single_infos["new_state"]

            obs_fixed.append(single_obs)
        obs_fixed = np.stack(obs_fixed)

        if self.venv.num_envs == 1:
            old_obs = np.array([self._old_obs[0]])
            obs_fixed = obs_fixed.reshape(old_obs.shape)
            rews = self.reward_fn(old_obs, self._actions, obs_fixed, np.array(dones))
        else:
