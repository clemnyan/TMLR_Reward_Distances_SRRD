...
# If CPLEX is chosen and a cplex_model is passed in to constructOptimization()
#   then docplex API is used instead of txt files for processing.
#
# Notation:
...
def lowerboundR(state_idx1, action_idx, state_idx2, peak, cplex_model = None):
    if cplex_model == None:
        return ("{}: {};".format(genR(state_idx1, action_idx, state_idx2), -peak))
    else:
        return (-peak)

def upperboundR(state_idx1, action_idx, state_idx2, peak, cplex_model = None):
    if cplex_model == None:
        return ("{}: {};".format(genR(state_idx1, action_idx, state_idx2), peak))
    else:
        return (peak)

def lowerboundFRV_targ(state_idx1, action_idx, state_idx2, peak, cplex_model = None):
    if cplex_model == None:
        return ("{}:{};".format(genFRV_targ(state_idx1, action_idx, state_idx2), -peak * 2.0))
    else:
        return (-peak * 2.0)

def upperboundFRV_targ(state_idx1, action_idx, state_idx2, peak, cplex_model = None):
    if cplex_model == None:
        return ("{}:{};".format(genFRV_targ(state_idx1, action_idx, state_idx2), peak * 2.0))
    else:
        return (peak * 2.0)

def lowerboundFRV_nontarg(state_idx1, action_idx, state_idx2, peak, cplex_model = None):
    if cplex_model == None:
        return ("{}:{};".format(genFRV_nontarg(state_idx1, action_idx, state_idx2), -peak * 2.0))
    else:
        return (-peak * 2.0)

def upperboundFRV_nontarg(state_idx1, action_idx, state_idx2, peak, cplex_model = None):
    if cplex_model == None:
        return ("{}:{};".format(genFRV_nontarg(state_idx1, action_idx, state_idx2), peak * 2.0))
    else:
        return (peak * 2.0)

def lowerbounddelta(poset_idx1, poset_idx2, cplex_model = None):
    if cplex_model == None:
        return ("{}: 1;".format(gendelta(poset_idx1, poset_idx2)))
    else:
        return (1)

def lowerboundLER(traj_idx, cplex_model):
    return (-cplex.infinity)

def lowerboundLB(poset_idx, cplex_model):
    return (-cplex.infinity)

def lowerboundUB(poset_idx, cplex_model):
    return (-cplex.infinity)
...
def constraintLER(traj_idx, trajectory, P, cplex_model = None):
    if cplex_model == None:
        constraint = genLER(traj_idx)
        for state_pos in range(floor(len(trajectory) / 2)):

            constraint += " - {} * {}".format(P[(trajectory[(state_pos + \
                1) * 2], trajectory[state_pos * 2 + 1], trajectory[state_pos * \
                2])], genR(trajectory[state_pos * 2], trajectory[state_pos * \
                2 + 1], trajectory[(state_pos + 1) * 2]))

        constraint += " == 0;"
        return (constraint)
    else:
        maps = dict()
        maps[genLER(traj_idx)] = 1.0
        for state_pos in range(floor(len(trajectory) / 2)):
            try:
                maps[genR(trajectory[state_pos * 2], trajectory[state_pos * \
                    2 + 1], trajectory[(state_pos + 1) * 2])] += \
                    -P[(trajectory[(state_pos + 1) * 2], trajectory[state_pos * \
                     2 + 1], trajectory[state_pos * 2])]
            except KeyError:
                maps[genR(trajectory[state_pos * 2], trajectory[state_pos * 2 + \
                    1], trajectory[(state_pos + 1) * 2])] = \
                    -P[(trajectory[(state_pos + 1) * 2], trajectory[state_pos * \
                     2 + 1], trajectory[state_pos * 2])]
        inds = list(maps.keys())
        vals = list(maps.values())
        return((cplex.SparsePair(ind = inds, val = vals), 0, 'E', \
            constraintLER_name(traj_idx)))
...
def constraintFRV_targ(state_idx1, action_idx, state_idx2, peak, p, \
    ...
    if cplex_model == None:
        return ("{} - {} == {};".format(genR(state_idx1, action_idx, \
            state_idx2), genFRV_targ(state_idx1, action_idx, state_idx2), peak * p))
    else:
        inds = [ genR(state_idx1, action_idx, state_idx2), \
            genFRV_targ(state_idx1, action_idx, state_idx2) ]

        vals = [ 1.0, -1.0 ]

        return((cplex.SparsePair(ind = inds, val = vals), peak * p, 'E', \
            constraintFRV_targ_name(state_idx1, action_idx, state_idx2)))

#   "FRV_targ_[s_{},a_{},s_{}]".format(i, j, i') <= "FRV_maxMagnitude"
def constraintFRV_targ_magnitude1_name(state_idx1, action_idx, state_idx2):
    desc = "_FRV_TARG_{}_{}_{}_maxMagnitude1".format(state_idx1, action_idx, \
        state_idx2)
...
def constraintFRV_targ_magnitude1(state_idx1, action_idx, state_idx2, \
    ...
    if cplex_model == None:
        return ("{} - {} <= 0;".format(genFRV_targ(state_idx1, action_idx, \
            state_idx2), genFRV_maxMagnitude()))
    else:
        inds = [ genFRV_targ(state_idx1, action_idx, state_idx2), \
            genFRV_maxMagnitude() ]
        vals = [ 1.0, -1.0 ]

        return((cplex.SparsePair(ind = inds, val = vals), 0, 'L', \
            constraintFRV_targ_magnitude1_name(state_idx1, action_idx, \
                state_idx2)))

#   -"FRV_targ_[s_{},a_{},s_{}]".format(i, j, i') <= "FRV_maxMagnitude"
def constraintFRV_targ_magnitude2_name(state_idx1, action_idx, state_idx2):
    desc = "_FRV_TARG_{}_{}_{}_maxMagnitude2".format(state_idx1, action_idx, \
        state_idx2)
...
def constraintFRV_targ_magnitude2(state_idx1, action_idx, state_idx2, \
    ...
    if cplex_model == None:
        ...
        vals = [ -1.0, -1.0 ]
        return((cplex.SparsePair(ind = inds, val = vals), 0, 'L', \
            constraintFRV_targ_magnitude2_name(state_idx1, action_idx, \
                state_idx2)))
...
def constraintFRV_nontarg(state_idx1, action_idx, state_idx2, peak, p, \
    ...
    if cplex_model == None:
        return ("{} - {} == {};".format(genR(state_idx1, action_idx, \
            state_idx2), genFRV_nontarg(state_idx1, action_idx, state_idx2), P))
    else:
        inds = [ genR(state_idx1, action_idx, state_idx2), \
            genFRV_nontarg(state_idx1, action_idx, state_idx2) ]
        vals = [ 1.0, -1.0 ]
        return ((cplex.SparsePair(ind = inds, val = vals), P, 'E', \
            constraintFRV_nontarg_name(state_idx1, action_idx, state_idx2)))

#   "FRV_nontarg_[s_{},a_{},s_{}]".format(i, j, i') <= "FRV_maxMagnitude"
def constraintFRV_nontarg_magnitude1_name(state_idx1, action_idx, state_idx2):
    desc = "_FRV_NONTARG_{}_{}_{}_maxMagnitude1".format(state_idx1, action_idx, \
        state_idx2)
...
def constraintFRV_nontarg_magnitude1(state_idx1, action_idx, state_idx2, \
    cplex_model = None):
    if cplex_model == None:
        return ("{} - {} <= 0;".format(genFRV_nontarg(state_idx1, action_idx, \
            state_idx2), genFRV_maxMagnitude()))
    else:
        inds = [ genFRV_nontarg(state_idx1, action_idx, state_idx2), genFRV_maxMagnitude() ]
        vals = [ 1.0, -1.0 ]
        return ((cplex.SparsePair(ind = inds, val = vals), 0, 'L', \
            constraintFRV_nontarg_magnitude1_name(state_idx1, action_idx, \
                state_idx2)))

#   -"FRV_nontarg_[s_{},a_{},s_{}]".format(i, j, i') <= "FRV_maxMagnitude"
def constraintFRV_nontarg_magnitude2_name(state_idx1, action_idx, state_idx2):
    desc = "_FRV_NONTARG_{}_{}_{}_maxMagnitude2".format(state_idx1, action_idx, \
        state_idx2)
...
def constraintFRV_nontarg_magnitude2(state_idx1, action_idx, state_idx2, \
    ...
    if cplex_model == None:
        ...
        inds = [genFRV_nontarg(state_idx1, action_idx, state_idx2), \
            genFRV_maxMagnitude() ]
        vals = [ -1.0, -1.0 ]
        return ((cplex.SparsePair(ind = inds, val = vals), 0, 'L', \
            constraintFRV_nontarg_magnitude2_name(state_idx1, action_idx, \
                state_idx2)))
...
def constraintPosetLBUB(poset_idx, cplex_model = None):
    if cplex_model == None:
        return ("{} - {} <= 0;".format(genLB(poset_idx), genUB(poset_idx)))
    else:
        inds = [genLB(poset_idx), genUB(poset_idx) ]
        vals = [ 1.0, -1.0 ]
        return ((cplex.SparsePair(ind = inds, val = vals), 0, 'L', \
            constraintPosetLBUB_name(poset_idx)))
...
def constraintPoset(poset_idx1, poset_idx2, cplex_model = None):

    delta_const = -50

    if cplex_model == None:
        return ("{} - {} - {} == 0;".format(genLB(poset_idx1), \
            genUB(poset_idx2), gendelta(poset_idx1, poset_idx2)))
    else:
        inds = [ genLB(poset_idx1), genUB(poset_idx2), gendelta(poset_idx1, \
            poset_idx2) ]
        #vals = [ 1.0, -1.0, -1.0 ]

        vals = [ 1.0, -1.0, delta_const]  #modified by Clement
        return ((cplex.SparsePair(ind = inds, val = vals), 0, 'E', \
            constraintPoset_name(poset_idx1, poset_idx2)))
...
def constraintPosetLB(poset_idx, traj_idx, cplex_model = None):
    if cplex_model == None:
        return ("{} - {} <= 0;".format(genLB(poset_idx), genLER(traj_idx)))
    else:
        inds = [ genLB(poset_idx), genLER(traj_idx) ]
        vals = [ 1.0, -1.0 ]
        return ((cplex.SparsePair(ind = inds, val = vals), 0, 'L', \
            constraintPosetLB_name(poset_idx, traj_idx)))
...
def constraintPosetUB(poset_idx, traj_idx, cplex_model = None):
    if cplex_model == None:
        return ("{} - {} >= 0;".format(genUB(poset_idx), genLER(traj_idx)))
    else:
        inds = [ genUB(poset_idx), genLER(traj_idx) ]
        vals = [ 1.0, -1.0 ]
        return ((cplex.SparsePair(ind = inds, val = vals), 0, 'G', \
            constraintPosetUB_name(poset_idx, traj_idx)))

#
# Declare all variables -- specific to BARON
def addVariable(var, var_type, binary_variables, integer_variables, \
    positive_variables, variables):
...
def declareVariables(num_states, num_actions, num_posets, trajectories, \
        dm_trajectory_indices, others_trajectory_indices, peak = None, \
        cplex_model = None):
    ...
    for t_idx in range(len(trajectories)):
        ...
        for pos in range(0, len(traj) - 1, 2):
            ...
            s_idx2 = traj[pos + 2]
            if cplex_model == None:
                addVariable(genR(s_idx1, a_idx, s_idx2), typeR(), \
                    binary_variables, integer_variables, positive_variables, \
                    variables)
            else:
                assert typeR() == "VARIABLE", "R variables only implemented as VARIABLE type."
                if typeR() == "VARIABLE":
                    name = genR(s_idx1, a_idx, s_idx2)
                    if not name in cplex_keys:
                        cplex_keys.add(name)
                        cplex_variables_list.append((lowerboundR(s_idx1, a_idx, \
                            s_idx2, peak, cplex_model), upperboundR(s_idx1, \
                            a_idx, s_idx2, peak, cplex_model), name))

            if t_idx in dm_trajectory_indices:
                if cplex_model == None:
                    addVariable(genFRV_targ(s_idx1, a_idx, s_idx2), \
                        typeFRV_targ(), binary_variables, integer_variables, \
                        positive_variables, variables)
                else:
                    assert typeFRV_targ() == "VARIABLE", "FRV_targ variables only implemented as VARIABLE type."
                    name = genFRV_targ(s_idx1, a_idx, s_idx2)
                    if not name in cplex_keys:
                        cplex_keys.add(name)
                        cplex_variables_list.append((lowerboundFRV_targ(s_idx1, a_idx, s_idx2, peak, cplex_model), upperboundFRV_targ(s_idx1, a_idx, s_idx2, peak, cplex_model), name))
            if t_idx in others_trajectory_indices:
                if cplex_model == None:
                    ...
                    assert typeFRV_nontarg() == "VARIABLE", "FRV_nontarg variables only implemented as VARIABLE type."
                    name = genFRV_nontarg(s_idx1, a_idx, s_idx2)
                    if not name in cplex_keys:
                        cplex_keys.add(name)
                        cplex_variables_list.append((lowerboundFRV_nontarg(s_idx1, a_idx, s_idx2, peak, cplex_model), upperboundFRV_nontarg(s_idx1, a_idx, s_idx2, peak, cplex_model), name))

    if cplex_model == None:
        addVariable(genFRV_maxMagnitude(), typeFRV_maxMagnitude(), binary_variables, integer_variables, positive_variables, variables)
    else:
        assert typeFRV_maxMagnitude() == "POSITIVE_VARIABLE", "FRV_maxMagnitude variable only implemented as POSITIVE_VARIABLE type."
        name = genFRV_maxMagnitude()
        if not name in cplex_keys:
            cplex_keys.add(name)
            cplex_variables_list.append((0, cplex.infinity, name))
    for t_idx, traj in enumerate(trajectories):
        ...
        if cplex_model == None:
            ...
            assert typeLER() == "VARIABLE", "LER variables only implemented as VARIABLE type."
            if not var_name in cplex_keys:
                cplex_keys.add(var_name)
                cplex_variables_list.append((lowerboundLER(t_idx, cplex_model), cplex.infinity, var_name))
    ...
    for p_idx in range(num_posets):
        if cplex_model == None:
            ...
            assert typeLB() == "VARIABLE", "LB variables only implemented as VARIABLE type."
            name = genLB(p_idx)
            if name not in cplex_keys:
                cplex_keys.add(name)
                cplex_variables_list.append((lowerboundLB(p_idx, cplex_model), cplex.infinity, name))

    for p_idx in range(num_posets):
        if cplex_model == None:
            addVariable(genUB(p_idx), typeUB(), binary_variables, integer_variables, positive_variables, variables)
        else:
            assert typeUB() == "VARIABLE", "UB variables only implemented as VARIABLE type."
            name = genUB(p_idx)
            if name not in cplex_keys:
                cplex_keys.add(name)
                cplex_variables_list.append((lowerboundUB(p_idx, cplex_model), cplex.infinity, name))
    ...
    for p_idx in range(num_posets - 1):
        if cplex_model == None:
            ...
            assert typedelta() == "POSITIVE_VARIABLE", "delta variable only implemented as POSITIVE_VARIABLE type."
            name = gendelta(p_idx, p_idx + 1)
            if name not in cplex_keys:
                cplex_keys.add(name)
                cplex_variables_list.append((lowerbounddelta(p_idx, p_idx + 1, cplex_model), cplex.infinity, name))
    ...
    if cplex_model == None:
        ...
        lbs, ubs, keys = zip(*cplex_variables_list)
        keys = list(keys)
        cplex_variables_idx = cplex_model.variables.add(names = keys, lb = list(lbs), ub = list(ubs))
        cplex_variables = dict(zip(keys, cplex_variables_idx))

    if DEBUG:
        print ('Total variables created = {}'.format(len(var_to_var_desc)))
    return (( rows, LER_to_trajectory ))

#
# Bound variables
def boundVariables(num_states, num_actions, peak, trajectories, num_posets, dm_trajectory_indices, others_trajectory_indices, optimizer, cplex_model = None):
    ...
    if optimizer == "CPLEX":
        if cplex_model != None:
            return (None) # Already taken care of in declareVariables()
...
#
# Build constraints
def generateConstraints(num_states, num_actions, peak, posets, trajectories, \
        P, dm_trajectory_indices, others_trajectory_indices, P_targ, P_nontarg, \
        optimizer, cplex_model = None):
    ...
    for t_idx in range(len(trajectories)):
        ...
        if not eqn in equation_names_set:
            ...
            equations.append(constraintLER(t_idx, trajectories[t_idx], P, cplex_model))
            num_constraints += 1
        ...
        for pos in range(0, len(traj) - 1, 2):
            ...
            if t_idx in dm_trajectory_indices:
                ...
                if not eqn in equation_names_set:
                    equation_names_set.add(eqn)
                    equation_names.append(eqn)
                    equations.append(constraintFRV_targ(s_idx1, a_idx, s_idx2, peak, P_targ[(a_idx, s_idx1)], cplex_model))
                    num_constraints += 1
                ...
                if not eqn in equation_names_set:
                    equation_names_set.add(eqn)
                    equation_names.append(eqn)
                    equations.append(constraintFRV_targ_magnitude1(s_idx1, a_idx, s_idx2, cplex_model))
                    num_constraints += 1
                eqn = constraintFRV_targ_magnitude2_name(s_idx1, a_idx, s_idx2)
                if not eqn in equation_names_set:
                    equation_names_set.add(eqn)
                    equation_names.append(eqn)
                    equations.append(constraintFRV_targ_magnitude2(s_idx1, a_idx, s_idx2, cplex_model))
                    num_constraints += 1
            if t_idx in others_trajectory_indices:
                ...
                if not eqn in equation_names_set:
                    equation_names_set.add(eqn)
                    equation_names.append(eqn)
                    equations.append(constraintFRV_nontarg(s_idx1, a_idx, s_idx2, peak, P_nontarg[(a_idx, s_idx1)], cplex_model))
                    #equations.append(constraintFRV_nontarg(s_idx1, a_idx, s_idx2, -1*peak, P_nontarg[(a_idx, s_idx1)], cplex_model))
                    num_constraints += 1
                ...
                if not eqn in equation_names_set:
                    equation_names_set.add(eqn)
                    equation_names.append(eqn)
                    equations.append(constraintFRV_nontarg_magnitude1(s_idx1, a_idx, s_idx2, cplex_model))
                    num_constraints += 1
                eqn = constraintFRV_nontarg_magnitude2_name(s_idx1, a_idx, s_idx2)
                if not eqn in equation_names_set:
                    equation_names_set.add(eqn)
                    equation_names.append(eqn)
                    equations.append(constraintFRV_nontarg_magnitude2(s_idx1, a_idx, s_idx2, cplex_model))
                    num_constraints += 1
    ...
    for p_idx in range(len(posets)):
        ...
        if not eqn in equation_names_set:
            equation_names_set.add(eqn)
            equation_names.append(eqn)
            equations.append(constraintPosetLBUB(p_idx, cplex_model))
            num_constraints += 1
    for p_idx in range(len(posets) - 1):
        ...
        if not eqn in equation_names_set:
            equation_names_set.add(eqn)
            equation_names.append(eqn)
            equations.append(constraintPoset(p_idx, p_idx + 1, cplex_model))
            num_constraints += 1
    ...
    for p_idx, poset in enumerate(posets):
        for t_idx in poset:
            ...
            if not eqn in equation_names_set:
                equation_names_set.add(eqn)
                equation_names.append(eqn)
                equations.append(constraintPosetLB(p_idx, t_idx, cplex_model))
                num_constraints += 1
    ...
    for p_idx, poset in enumerate(posets):
        for t_idx in poset:
            ...
            if not eqn in equation_names_set:
                equation_names_set.add(eqn)
                equation_names.append(eqn)
                equations.append(constraintPosetUB(p_idx, t_idx, cplex_model))
                num_constraints += 1
    ...
    if cplex_model == None:
        ...
        lin_exprs, rhs, senses, names = zip(*equations)
        cplex_model.linear_constraints.add(lin_expr = list(lin_exprs), senses = list(senses), rhs = list(rhs), names = list(names))
    del equation_names
    del equations
    del equation_names_set
    return (rows)

def genObjective(num_posets, cplex_model = None):
#       max \sum_{q=0}{num_posets-1} "delta_{}_{}".format(q, q+1)
#       -- maximizes the distance between posets
#       -- can overfit trajectories
    if cplex_model == None:
...
#        inds = list()
#        vals = [1.0] * (num_posets - 1)
#        for p_idx in range(num_posets - 1):
#            inds.append(gendelta(p_idx, p_idx + 1))
#        cplex_model.objective.set_linear(zip(inds, vals))
#        cplex_model.objective.set_sense(cplex_model.objective.sense.maximize)
        cplex_model.objective.set_linear(genFRV_maxMagnitude(), 1.0)
...
def constructOptimization(**kwargs):
    ...
    try:
        cplex_model = kwargs["cplex_model"]
    except KeyError:
        cplex_model = None
...
#   dm_trajectory_indices are the trajectories to be used for P_{DM}. If None, assumes all trajectories are DM trajectories.
    ...
    P, P_targ, P_nontarg, _, _, _, _, _, _  = computePs(trajectories, \
    ...
    if optimizer == "BARON": # Generate problem in BARON format
        rows = [ "OPTIONS {", "MaxTime:{};".format(maxtime), \
            "LicName: \"{}\";".format(license_fn), "nlpsol: 10;", "}", " " ]
        if DEBUG:
            print ("Declaring variables...")

        ( new_rows, LER_to_trajectories ) = declareVariables(num_states, \
            num_actions, num_posets, trajectories, dm_trajectory_indices, \
        ...
        rows.extend(boundVariables(num_states, num_actions, peak, trajectories, \
            num_posets, dm_trajectory_indices, others_trajectory_indices, \
            optimizer))
        ...
        rows.extend(generateConstraints(num_states, num_actions, peak, posets, \
            trajectories, P, dm_trajectory_indices, others_trajectory_indices, \
            P_targ, P_nontarg, optimizer))

        rows.append(" ")
        rows.extend(genObjective(num_posets))
    if optimizer == "CPLEX": # Generate problem in CPLEX format
        ...
        if cplex_model == None:
            if DEBUG:
                print ("Declaring variables...")
            (new_row_M, LER_to_trajectories ) = declareVariables(num_states, \
                num_actions, num_posets, trajectories, dm_trajectory_indices, \
                others_trajectory_indices)
            ...
            newrows = generateConstraints(num_states, num_actions, peak, \
                posets, trajectories, P, dm_trajectory_indices, \
                others_trajectory_indices, P_targ, P_nontarg, optimizer)
            ...
            newrows = boundVariables(num_states, num_actions, peak, \
                trajectories, num_posets, dm_trajectory_indices, \
                others_trajectory_indices, optimizer)
            ...
            (new_row_M, LER_to_trajectories ) = declareVariables(num_states, \
                num_actions, num_posets, trajectories, dm_trajectory_indices, \
                others_trajectory_indices, peak, cplex_model)
            ...
            genObjective(num_posets, cplex_model)
            ...
            generateConstraints(num_states, num_actions, peak, posets, \
                trajectories, P, dm_trajectory_indices, \
                others_trajectory_indices, P_targ, P_nontarg, optimizer, \
                cplex_model)
    ...
    return (( rows, LER_to_trajectories, P, P_targ, P_nontarg, \
        copy.deepcopy(var_to_var_desc), cplex_variables ))
...
#   21  22  23  24  25
    ...
    rows = constructOptimization(num_states, num_actions, peak, num_posets, \
        posets, t, dm_trajectory_indices, "BARON", 100000, "./baronlice.txt")
    with open("tmp2-RoMDP.mnlp", "w") as f:
        for row in rows:
            f.write(row + '\n')
    os.system('baron tmp2-RoMDP.mnlp')
    answers = L_MDP.extractAnswers("soln2-RoMDP.csv", var_to_var_desc)


def rewrite_trajs (trajs, posets):
    num_agents = 2
    ...
    for p in posets[1]:
        ...
        for n in range(num_agents):
            ...
            for iter_t in t:
                temp.append(iter_t[n])
            non_target.append(temp)

    new_posets = []
    target_indices = [k for k in range(len(target))]
    non_target_indices = [k+len(target) for k in range(len(target))]
    new_posets = [target_indices, non_target_indices]

    return new_posets, target + non_target
...
#   21  22  23  24  25
    ...
    #t1 = [[0, 0], [1, 0], [1, 2], [0, 0], [2, 2]]
    #t2 = [[0, 0], [1, 1], [2, 1], [0, 1], [1, 0], [1, 0], [1, 2]]

    t1 = [[0, 2], [0, 1], [1, 2]]
    t2 = [[0, 0], [1, 1], [2, 1], [1, 0], [2, 1]]
    ...
    # after decomposing to individual level
    posets, t =  rewrite_trajs (trajs, posets)
    num_posets = 2
    num_states = 3
    num_actions = 2
    peak = 100
    ...
    rows, LER_to_trajectories, P, P_targ, P_nontarg, var_to_var_desc, \
        cplex_variables = constructOptimization(num_states = num_states, \
            num_actions = num_actions, tau = None, peak = peak, \
            num_posets = num_posets, posets = posets, all_trajectories = t, \
            dm_trajectory_indices = posets[0], \
            others_trajectory_indices = posets[1], optimizer = "CPLEX", \
            maxtime = 100000, license_fn = "./baronlice.txt")
