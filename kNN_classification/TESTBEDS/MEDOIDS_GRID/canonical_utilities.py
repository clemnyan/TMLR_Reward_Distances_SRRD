...
def manual_canonicalization(grid_rewards, state_rewards, grid_size, gamma):
    ...
    for fname in os.listdir(TRAJS_DIR):
        ...
        for traj in trajs_store:
            ...
            # perform regression
            #r_orig = perform_regression(r_orig, 20)
            #_epic = perform_regression(r_epic, 20)
            #r_dard = perform_regression(r_dard, 20)
            #r_sard = perform_regression(r_sard, 20)
...
def maxent_canonicalization(grid_size, gamma):
    ...
    for fname in os.listdir(TRAJS_DIR):
        ...
        for traj in trajs_store:
            ...
            traj_set = [k for k in set(traj)]

            # setup for maxent
            observation_matrix, state_mp, action_mp, env_single, om, \
                expert_trajs = mu.fix_for_irl([traj_set])
    ...
    for fname in os.listdir(TRAJS_DIR):
        ...
        for traj in list_chunks:
            ...
            for T in traj:
                for t in T:
                    s, a, sp = t
                    r = 2*rews[state_mp[s]] + rews[state_mp[sp]]
                    rew.append((s, a, sp, r))
...
def fully_connected_mdp (num_states, relationship):
    ...
    graph = {(s, a, sp): r for s, a, sp, r in graph if a < 4}   # graph is a dictionary
    return graph, states
...
def getcounts(reward_store, reward_labels, sample_points):
    ...
    for i in sample_points:
        ...
        dists = Counter([k[2] for k in sorted(dists)[:100]])
