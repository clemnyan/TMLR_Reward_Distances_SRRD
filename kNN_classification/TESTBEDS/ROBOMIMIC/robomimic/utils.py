...
def perform_AIRL(observation_matrix, env_single, expert_trajs, traj, \
    ...
    learner = PPO(env=env_single, policy=MlpPolicy, batch_size=128, \
        learning_rate=0.0001, n_epochs=5, seed=SEED, \
        observation_matrix = env_single.observation_matrix)

    airl_trainer = AIRL(demonstrations=expert_trajs, demo_batch_size=128, \
...
def perform_Maxent(state_mp, observation_matrix, env_single, pi, \
    ...
    occ_measure = mce_irl.train(max_iter = 500)

    return reward_net


def get_reward_models (traj_arrs, iterations, state_mp, TRAJ_DIV, rew_type):

    S_MP = {j:i for i, j in state_mp.items()}

    REW_MP = {}

    for i, T_set in enumerate(traj_arrs):
        if i not in REW_MP:
            REW_MP[i] = []
...
def return_top(d_arr, description):
    res1 = sum([1 for i, j in d_arr[:10] if j == True])/10*100
    res2 = sum([1 for i, j in d_arr[:20] if j == True])/20*100
    res3 = sum([1 for i, j in d_arr[:30] if j == True])/30*100
...
def get_dist(keys, REW_MP, agent_keys, all_keys, rew_type, gamma):
    ...
    d_arr.sort()
    #print("d_arr sorted = ", d_arr)

    d_arr_epic.sort()
    #print("d_arr epic sorted = ", d_arr_epic)

    d_arr_dard.sort()
    #print("d_arr_dard = ", d_arr_dard)

    d_arr_srrd.sort()
    #print("d_arr_srrd = ", d_arr_srrd)
