...
def fix_for_irl (trajs):
    ...
    for traj in trajs:
        ...
        for i, t in enumerate(traj):
            ...
            transition = (state_mp[s], action_mp[a], state_mp[sp])
    ...
    for t in t_mp:
        #print(t)
        s, a, sp = t
        #s, a, sp = state_mp[t[0]], action_mp[t[1]], state_mp[t[2]]
    ...
    for s in range(len(t_matrix)):
        ...
        for a in action_mp:
            #print('----', t_matrix.shape, a, action_mp[a])
            #for a in range(len(t_matrix[s])):
            row_sum = sum(t_matrix[s, action_mp[a]])
            if row_sum != 0:
                t_matrix[s, action_mp[a]] /= row_sum
            else:
                t_matrix[s, action_mp[a]] = 1/len(state_mp)
...
def perform_AIRL(env_single, expert_trajs, ITERATIONS = 10000, \
        is_linear = "nonlinear"):
