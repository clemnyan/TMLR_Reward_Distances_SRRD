...
def process_files (dir):
    RAW_DIR, NEW_DIR = 'RAW_GAME_DATA/', 'PROCESSED_DATA/'
...
# reset files everytime you run
if os.path.isdir("PROCESSED_DATA"):
    shutil.rmtree("PROCESSED_DATA")
if os.path.isfile("state_action_mps.pkl"):
    os.remove("state_action_mps.pkl")

# create files here
os.mkdir("PROCESSED_DATA")

# now do a run
dirs = os.listdir('RAW_GAME_DATA')
store = []
for dir_arr in divide_arr(dirs, len(dirs)//2 + 1):
    print("len dir_arr = ", len(dir_arr))
...
def re_write_mp (store):
    ...
    for arr in store:
        ...
        for prev_dir in prev_arr:
            ...
            folder_name, file_name = prev_dir.split('PROCESSED_DATA/')[1].split('/')
            if not os.path.isdir(new_dir + folder_name):
                os.mkdir(new_dir + folder_name)

            new_name = new_dir + prev_dir.split('PROCESSED_DATA/')[1]
